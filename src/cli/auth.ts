...
/** The same transport, once there is a token to send. */
export interface InitHttp extends DeviceApi {
...
export function deviceApi(baseUrl: string, token?: string): InitHttp {
  ...
  async function send(method: 'GET' | 'POST', path: string, body?: unknown): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        `${root} answered ${response.status} with something that is not JSON.`,
      );
    }
  ...
  for (;;) {
    ...
    switch (result.error) {
      ...
      default:
        ...
        throw new Error(
          `The server answered with something this version does not understand: ${result.error}`,
        );
    }
  }
}
